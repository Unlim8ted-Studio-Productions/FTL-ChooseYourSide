...
ftldat_entry = collections.namedtuple("ftldat_entry", ("filename", "size", "offset"))
...
class FolderPack(object):
    ...
    def list_sizes(self):
        for filename in self.list():
            yield (
                filename,
                os.stat(os.path.join(self.root, *ftl_path_split(filename))).st_size,
            )
    ...
    def __contains__(self, filename):
        return os.path.exists(os.path.join(self.root, *ftl_path_split(filename)))
...
class FTLPack(object):
    ...
    def _read_index(self):
        ...
        for n in range(index_size):
            data = self.f.read(4)
            if not len(data) < 4:
                self.index[n] = struct.unpack("<L", data)[0]
        ...
        for n, offset in enumerate(self.index):
            ...
            data = self.f.read(8)
            if not len(data) < 8:
                size, lfn = struct.unpack("<LL", data)
                filename = self.f.read(lfn)
                self.metadata[n] = ftldat_entry(
                    filename=filename, size=size, offset=offset + 8 + len(filename)
                )
                if filename in self.filenames:
                    raise FTLDatError("Filename %s occurs more than once" % filename)
    ...
    def _grow_index(self, amount=1):
        ...
        while True:
            ...
            self._move_to_eof(n)
        # Update state
        self.index_free.extend(
            range(len(self.index) + free_room - 1, len(self.index) - 1, -1)
        )
    ...
    #
    def list(self):
        return iter(self.filenames.keys())
    ...
    def list_metadata(self):
        ...
        return [
            (n, x.filename, x.size, x.offset) for n, x in enumerate(self.metadata) if x
        ]
    ...
    def repack(self):
        ...
        entry = collections.namedtuple(
            "repack_entry", ("old_n", "old_offset", "filename", "size", "new_offset")
        )
        result = collections.namedtuple(
            "result", ("bytes_moved", "old_size", "new_size")
        )
        bytes_moved = 0
        old_size = self.eof
        entries = [
            ...
            for n, addr in enumerate(self.index)
            if addr
        ]
        entries.sort(key=lambda x: x.old_offset)
        # Check for overlap
        for i in range(len(entries) - 1):
            if entries[i].old_offset + entries[i].size > entries[i + 1].old_offset:
                raise FTLDatError("Cannot repack datfile with overlapping " + "entries")
        # Create new layout
        ...
        self.index_free = []
        return result(
            old_size=old_size, new_size=new_total_size, bytes_moved=bytes_moved
        )
...
class Program(object):
    ...
    def cmd_pack(self):
        if os.path.exists(self.args.datfile) and not self.args.force:
            print(("ERROR %s already exists. Use -f to override." % self.args.datfile))
            return -2
        ...
        try:
            if not self.args.filename in pack:
                if not self.args.force:
                    print(
                        (
                            "ERROR %s does not exist.  Use -f to add anyway"
                            % self.args.filename
                        )
                    )
                    return -9
    ...
    def cmd_add(self):
        ...
        if self.args.filename in pack:
            if not self.args.force:
                print(
                    ("ERROR %s already exists. Use -f to replace." % self.args.filename)
                )
                return -2
            pack.remove(self.args.filename)
    ...
    def cmd_extract(self):
        ...
        if (
            self.args.target
            and os.path.exists(self.args.target)
            and not self.args.force
        ):
            print(("ERROR %s already exists.  Use -f to override." % self.args.target))
            return -4
        if not self.args.filename in pack:
    ...
    def cmd_repack(self):
        ...
        res = pack.repack()
        print()
        print(" old size      %s (%s)" % (nice_size(res.old_size), res.old_size))
        print(
            " new size      %s (%s; %s%%)"
            % (
                nice_size(res.new_size),
                res.new_size,
    ...
    def parse_args(self):
        ...
        parser_info = subparsers.add_parser(
            "info", help="Shows detailed technical information about a datfile"
        )
        parser_info.add_argument("datfile", help="The datfile to examine")
        parser_info.add_argument(
            "--hashes", "-H", action="store_true", help="Show MD5 hashes"
        )
        parser_info.add_argument(
            "--bytes", "-B", action="store_true", help="Show sizes in bytes"
        )
        parser_info.set_defaults(func=self.cmd_info)

        parser_pack = subparsers.add_parser(
            "pack", help="Creates a datfile from a folder"
        )
        parser_pack.add_argument("datfile", help="The datfile to create")
        ...
        parser_pack.add_argument(
            "--indexsize", "-I", default=None, type=int, help="Index size."
        )
        parser_pack.add_argument(
            "-f", "--force", action="store_true", help="Override existing datfile"
        )
        parser_pack.set_defaults(func=self.cmd_pack)

        parser_unpack = subparsers.add_parser(
            "unpack", help="Unpacks a datfile to a folder"
        )
        parser_unpack.add_argument("datfile", help="The datfile to unpack")
        ...
        parser_add = subparsers.add_parser("add", help="Add a single file to a datfile")
        parser_add.add_argument("datfile", help="The datfile to add the file to")
        ...
        parser_append = subparsers.add_parser(
            "append", help="Append a file to an existing file in the datfile"
        )
        parser_append.add_argument("datfile", help="The datfile")
        parser_append.add_argument("filename", help="The file to append to")
        parser_append.add_argument("appendix", help="The file to append")
        ...
        parser_replace = subparsers.add_parser(
            "replace", help="Replace a single file in the datfile"
        )
        parser_replace.add_argument(
            "datfile", help="The datfile to replace the file to"
        )
        parser_replace.add_argument(
            "replacement", help="The file to replace [filename] with"
        )
        parser_replace.add_argument(
            "filename", help="The filename of the file to replace in the datfile"
        )
        parser_replace.add_argument(
            "-f",
            "--force",
        ...
        parser_remove = subparsers.add_parser(
            "remove", help="Removes a file from the datfile"
        )
        parser_remove.add_argument(
            "datfile", help="The datfile to remove the file from"
        )
        parser_remove.add_argument(
            "filename", help="The filename of the file to remove"
        )
        parser_remove.add_argument(
            "-f",
            "--force",
        ...
        parser_extract = subparsers.add_parser(
            "extract", help="Extract a single file from a datfile"
        )
        parser_extract.add_argument("datfile", help="The datfile to extract from")
        parser_extract.add_argument(
            "filename", help="The filename of the file to extract"
        )
        parser_extract.add_argument(
            "target",
            nargs="?",
        ...
        parser_hashes = subparsers.add_parser(
            "hashes",
            help="Shows the filenames and their md5 hashes alphabetically."
            + "  Useful for debugging.",
        )
        parser_hashes.add_argument("datfile", help="The datfile to examine")
        parser_hashes.set_defaults(func=self.cmd_hashes)

        parser_repack = subparsers.add_parser(
            "repack", help="Repacks the datfile: removes overhead"
        )
        parser_repack.add_argument("datfile", help="The datfile to repack")
        parser_repack.set_defaults(func=self.cmd_repack)

        parser_list = subparsers.add_parser(
            "list", help="Lists the filenames in the datfile"
        )
        parser_list.add_argument("datfile", help="The datfile to examine")
